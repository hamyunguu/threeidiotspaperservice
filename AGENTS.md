...
포스터 미리보기는 민구홍 매뉴팩처링 모크업 생성기의 조작을 참고한 별도 스튜디오
표현입니다. 어두운 배경·그림자·말림·펄럭임·자동 회전은 포스터에만 적용합니다.
말림과 펄럭임은 미리보기 설정이며 주문 사양에 저장하지 않습니다. 펄럭임은
표면 변형 애니메이션과 감쇠 진동으로 구현했으며 실제 용지 물성 시뮬레이션은 아닙니다.
상품 전환 시 캔버스 높이가 바뀌므로 `configurePreview` 후 `resize`를 유지하세요.
