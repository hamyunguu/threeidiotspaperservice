...
| `apply.html` + `apply.js` | 프로그램 신청 플로우 가안. 아카이브의 `?p=` 선택을 이어받음 |
...
- **떠다니는 구(T·i·P·S)** — 프로그램 시안에 없지만 프로그램과 아카이브 모두에
  남겼습니다. 페이지 고유가 아니라 사이트의 장치입니다. 아카이브 상세창이 열리면
  갤러리와 구를 포함한 뒤쪽 화면 전체가 블러 처리됩니다. 블러는 클릭 팝업과 같이
  즉시 적용하고 흰색 팝업만 따로 페이드되도록 열립니다
- **아카이브 좌상단** — 재단 마크는 있고 타깃(⊕)만 없습니다. 시안이 그렇습니다
  (뒤로 가기 화살표가 그 자리라서)
- **카트의 부가세 / 공급가액** — 시안에 라벨이 서로 바뀐 것으로 보입니다
  (`부가세 14.300` / `공급가액 1.430`). 합계는 맞아서 **시안 그대로** 두었습니다
- **서비스의 번호와 짧은 선택지 간격** — 공통 영역 시안은 번호가 18px / 간격이
  4px이지만, 상세 영역과 화면 전체의 위계를 맞추라는 피드백에 따라 모두 22px / 9px로
  통일했습니다. 양·단면, 판형 방향, 주문 일정은 색상 도수와 같은 22px 열 간격입니다
- **로그인 폼** — 계정 서버가 없어 `preventDefault()`로 붙잡습니다. 그냥 두면
  아이디와 비밀번호가 주소창에 실립니다

### 풀기 마크는 두 페이지에서 같은 원 위치를 씁니다

현재 Figma의 아카이브 상세(476:1390)와 프로그램 카드는 모두
`0 · 35 · 70 · 105 · 208`을 씁니다. 예전 아카이브 값
`0 · 28 · 56 · 120 · 208`은 더 이상 사용하지 않습니다.
...
- **아카이브 `fitStage()`는 65를 더 얹습니다.** 헤더를 열면 갤러리째 내려가는데
  여유가 없으면 마지막 줄이 잘립니다
- **아카이브 도련 표시는 화면을 따라다니지 않습니다.** 위쪽 표시와 뒤로 가기는
  페이지 상단에, 아래쪽 표시는 긴 갤러리의 끝에 붙습니다. 스크롤 중간에는 코너 표시가
  보이지 않는 것이 의도한 동작입니다
- **아카이브 본문은 함께 나타납니다.** 정적 HTML에 먼저 있는 `신청하러 가기`만
  번쩍이지 않도록 세션 마크·설명·버튼·첫 갤러리 줄·구를 `.arc-reveal`로 묶었습니다.
  첫 줄 이미지가 준비되거나 1.2초가 지나면 한 번에 표시합니다
- **프린트 팁은 `script.js`의 `TIPS` 스무 개**에서 뽑습니다. 원 하나가 팁 하나를
  갖는 것이 아니라, 열 때마다 다른 것이 나옵니다. 팁을 고치거나 더할 때는
  **줄바꿈을 직접 넣어야 합니다** — 카피 폭이 317이고 원이 둥글어서, 한 줄이라도
  넘치면 모서리에서 잘립니다. 본문은 다섯 줄까지
- **서비스 주문서는 판형 방향까지 필수입니다.** 규격을 고른 뒤 세로형/가로형을
  골라야 크기 단계가 완료됩니다. 종이 평량은 태산인디고 포스터·책자·리플렛·명함
  상품이 반환하는 용지별 옵션을 `order.js`의 `PAPER_WEIGHTS`에 옮긴 것이므로,
  거래처 옵션이 바뀌면 상품별로 다시 확인해야 합니다. Products의 세부 종류를
  바꾸면 종이와 평량도 해당 상품 기준으로 초기화됩니다.
- **서비스에서 카트로 넘기는 주문은 세션 초안입니다.** `tips-cart-order`라는
  `sessionStorage` 주문 정보와 `tips-cart-preview` 미리보기를 쓰며, 탭을 닫으면
  사라집니다. 첫 업로드 PDF의 첫 페이지는 로드할 때 바로 512px 이하 JPEG 썸네일로
  줄여 별도 저장하고 카트의 작업물 칸에 표시합니다. 주문 JSON과 이미지를 분리한 것은
  브라우저 저장 용량 때문에 이미지와 주문 전체가 함께 유실되는 것을 막기 위해서입니다.
  결제·주문 접수 서버는 아직 없으며 주문 데이터는 외부에 저장하거나 전송하지 않습니다.
- **카트 주문 버튼은 현장용 80mm 주문 접수증을 인쇄합니다.** 날짜별 접수번호는
  현장 브라우저의 `localStorage`에서 증가하고, 발급한 접수증은 같은 탭의
  `sessionStorage`에 남아 재인쇄할 수 있습니다. 새 주문을 카트에 담으면 이전 접수증은
  폐기됩니다. 여러 현장 컴퓨터를 쓰면 번호가 각각 증가하므로 한 대를 접수 전용으로
  사용해야 합니다. 결제 서버가 없어 접수증에는 `결제 전`과 `결제 영수증 아님`을
  명시합니다. 일반 브라우저에서는 인쇄 창이 열리며, 무인 즉시 출력은 현장 Chrome을
  kiosk printing 모드로 설정해야 합니다.
...
포스터 미리보기는 민구홍 매뉴팩처링 모크업 생성기의 첫 화면을 참고해 50% 수준의
펄럭임을 고정한 별도 표현입니다. 조절 UI·바닥·그림자 없이 흰색 배경과 렌더만
표시합니다. 초기 규격은 세로 A2(420 × 594mm)입니다. 펄럭임은 표면 변형 애니메이션과
감쇠 진동으로 구현했으며 실제 용지 물성 시뮬레이션은 아닙니다. 상품 전환 시 캔버스 높이가 바뀌므로
`configurePreview` 후 `resize`를 유지하세요.
포스터·리플렛·명함의 표면은 인쇄 내용 확인을 우선합니다. 무광은 거칠기 1·반사 0.1·
코팅층 0이며, 유광도 거칠기 0.42·반사 0.25·코팅층 0.06으로 반사광을 제한합니다.
표면의 `roughnessMap`·종이결 노멀·sheen은 쓰지 않습니다. 특히 회색 roughnessMap은
거칠기를 절반으로 낮춰 검은 작업물에 흰 반사 얼룩을 만들므로 다시 붙이지 마세요.

책 미리보기의 기본 모델은 민구홍 매뉴팩처링 모크업 생성기의 책 첫 화면을 참고합니다.
페이지 수를 입력하기 전에는 20mm 샘플 책등을 보여 주고, 입력 후에는 계산된 세네카로
바뀝니다. 무선·PUR·사철은 평평한 책등과 3mm 위치의 1mm 제본 홈을 사용합니다.
표지 표면은 반사·종이결·코팅이 없는 기본 무광이며, 주광은 월드 좌표 `(5, 5, 5)`에
고정합니다. OrbitControls의 입력 카메라와 실제 렌더 카메라를 분리하고, 물체 그룹을
회전시켜 같은 면도 빛과 이루는 각도에 따라 명암이 바뀝니다. 투영 그림자는 만들지 않습니다. 책과
포스터는 위·아래까지 자유롭게 회전하도록 polar 범위를 거의 0~π로 열어 둡니다.
Safari가 포커스된 WebGL 캔버스에 파란 외곽선을 만들므로 `.ord-canvas`의 outline을
제거합니다. 포스터의 고정 50% 펄럭임은 진폭 계수 0.034, 속도 계수 0.78을 사용합니다.
...
### 프로그램 신청 페이지도 아직 가안입니다

`apply.html?p=1|2|3`은 일정·회차 → 신청자 정보 → 약관 확인까지 전체 흐름을
시험하기 위한 화면입니다. 예시 일정과 입력값은 브라우저 메모리에만 있고 어디에도
저장하거나 전송하지 않습니다. 실제 운영 전에는 일정·참가비·취소 규정·개인정보
처리 문구를 확정하고 접수 서버 또는 예약 서비스를 연결해야 합니다.
