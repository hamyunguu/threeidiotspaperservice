...
- **서비스 3D는 무채색 재질 보기입니다.** 사용자 요청으로 업로드 이미지·용지 색은
  모델에 표시하지 않고, 미리보기 설명·재단 마크·조작 버튼도 숨겼습니다. 업로드와 주문
  데이터는 유지합니다. 바닥과 배경 없이 투명 캔버스에 모델만 표시하며,
  스튜디오 환경광은 표면 조명에만 사용합니다.
  포스터는 기울어진 시점으로 표시하며 클릭·터치하면 `paper-physics.js`의 감쇠 탄성 모델이
  반응합니다. 평량·두께 기반 추정값이며 실측 종이 시뮬레이션이나 인쇄 색상 교정은 아닙니다.
  책장 넘김은 제본부 스프링으로 계산하고, 펼친 책장을 직접 드래그할 수 있습니다.
